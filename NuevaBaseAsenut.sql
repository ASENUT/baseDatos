/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     19/12/2016 19:22:44                          */
/*==============================================================*/
...
(
   CED_EMPLEADO         int not null AUTO_INCREMENT,
...
(
   COD_FICHA            char(10) not null AUTO_INCREMENT,
   FECHA_TRATA          date,
   COD_TIPO_PAGO        int,
...
(
   ...
   FECHA                date not null,
   primary key (FECHA_TRATA, COD_ANTROPOMETRICO)
);
...
alter table ACTIVIDAD_FISICA add constraint FK_REALIZA foreign key (FECHA_TRATA)
      references PACIENTE_TRATAMIENTO (FECHA_TRATA) on delete restrict on update restrict;
...
alter table FICHA_ECONOMICA add constraint FK_PAGADO foreign key (FECHA_TRATA)
      references PACIENTE_TRATAMIENTO (FECHA_TRATA) on delete restrict on update restrict;
...
alter table HISTORIA_CLINICA add constraint FK_TENER_HISTORIA foreign key (FECHA_TRATA)
      references PACIENTE_TRATAMIENTO (FECHA_TRATA) on delete restrict on update restrict;
...
alter table MEDIDAS_CORPORALES add constraint FK_MIDE foreign key (FECHA_TRATA)
      references PACIENTE_TRATAMIENTO (FECHA_TRATA) on delete restrict on update restrict;
...
alter table POSEE add constraint FK_POSEE2 foreign key (FECHA_TRATA)
      references PACIENTE_TRATAMIENTO (FECHA_TRATA) on delete restrict on update restrict;
